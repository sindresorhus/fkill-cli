...
const filterByPort = (processes, input) => processes.filter(process_ => process_.ports.map(port => String(port).includes(input)).some(v => v));

const filterProcesses = (input, processes, flags) => {
	const memoryThreshold = flags.verbose ? 0 : 1;
	const cpuThreshold = flags.verbose ? 0 : 3;

	const filteredProcesses = input.startsWith(':') ? filterByPort(processes, input.slice(1)) : new FuzzySearch(
		processes,
