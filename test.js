...
test('main', async t => {
	const {stdout} = await execa('./distribution/cli.js', ['--version']);
	t.true(stdout.length > 0);
});

test('pid', async t => {
	const pid = await noopProcess();
	await execa('./distribution/cli.js', ['--force', pid]);
...
if (process.platform === 'darwin') {
	test('fuzzy search', async t => {
		...
		await execa('./distribution/cli.js', ['o00oop@']);
		await noopProcessKilled(t, pid);
	});
}
...
test('error when process is not found', async t => {
	await t.throwsAsync(
		execa('./distribution/cli.js', ['--force', 'notFoundProcess']),
...
test('force killing process at unused port throws error', async t => {
	await t.throwsAsync(
		execa('./distribution/cli.js', ['--force', ':1337']),
...
test('silently force killing process at unused port exits with code 0', async t => {
	const {exitCode} = await execa('./distribution/cli.js', ['--force', '--silent', ':1337']);
	t.is(exitCode, 0);
});
